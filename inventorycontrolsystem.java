...
private static final String PURCHASE_HISTORY_FILE =
...
private static void removeProduct(Scanner scanner, List<Product>
products) {
...
private static void purchaseProduct(Scanner scanner, List<Product>
products, List<Purchase> purchaseHistory) {
...
Purchase purchase = new Purchase(product.getName(), quantity,
totalPrice, new Date());
...
while ((line = reader.readLine()) != null) {
String[] data = line.split(DELIMITER);
purchaseHistory.add(new Purchase(data[0],
Integer.parseInt(data[1]), Double.parseDouble(data[2]),
new Date(Long.parseLong(data[3]))));
}
} catch (IOException | NumberFormatException e) {
System.err.println("Error loading purchase history: " +
e.getMessage());
}
return purchaseHistory;
}
private static void savePurchaseHistoryToFile(List<Purchase>
purchaseHistory) {
try (BufferedWriter writer = new BufferedWriter(new
FileWriter(PURCHASE_HISTORY_FILE))) {
for (Purchase purchase : purchaseHistory) {
writer.write(purchase.getProductName() + DELIMITER +
purchase.getQuantity() + DELIMITER
+ purchase.getTotalPrice() + DELIMITER +
purchase.getPurchaseDate().getTime());
writer.newLine();
}
} catch (IOException e) {
System.err.println("Error saving purchase history: " +
e.getMessage());
}
}
}
...
public Purchase(String productName, int quantity, double totalPrice, Date
purchaseDate) {
...
@Override
public String toString() {
return "Product: " + productName + ", Quantity: " + quantity + ", Total
Price: $" + totalPrice + ", Purchase Date: "
+ purchaseDate;
}
}
